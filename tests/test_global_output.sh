...
(cd "$tmp" && "$root/dqmc" input.in > stdout.txt)
fail=0
...
(cd "$tmp" && "$root/dqmc" input.in > stdout2.txt)
...
(cd "$tmp" && "$root/dqmc" binonly.in > binonly.stdout)
...
if (cd "$tmp" && "$root/dqmc" input.in > /dev/null 2>&1); then echo "FAIL open error ignored"; fail=1; fi
...
collide() { # description extra_lines protected_file
  ...
  if (cd "$tmp" && "$root/dqmc" c.in > /dev/null 2> c.err); then echo "FAIL collision accepted: $1"; fail=1; fi
...
(cd "$tmp" && "$root/dqmc" d.in > /dev/null) || { echo "FAIL inactive default name rejected"; fail=1; }
...
for hook in AFQMC_TEST_BIN_WRITE_FAIL AFQMC_TEST_BIN_CLOSE_FAIL; do
  if (cd "$tmp" && env "$hook=1" "$root/dqmc" e.in > /dev/null 2>&1); then echo "FAIL $hook ignored"; fail=1; fi
done
# a numerical failure in a global pass of beta index 1 leaves no rows of that beta (I6)
rm -f "$tmp/bins2.tsv"
if (cd "$tmp" && AFQMC_TEST_GLOBAL_FAIL_AT=9 AFQMC_TEST_GLOBAL_FAIL_BETA=1 "$root/dqmc" e.in > numerical.stdout 2> numerical.err); then echo "FAIL numerical failure ignored"; fail=1; fi
